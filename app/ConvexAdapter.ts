...
export const ConvexAdapter: Adapter = {
  ...
  async getAccount(providerAccountId: string, provider: string) {
    return await callQuery(api.authAdapter.getAccount as any, {
      provider,
      providerAccountId,
    });
  },
  ...
  async linkAccount(account: Account) {
    return await callMutation(api.authAdapter.linkAccount as any, { account });
  },
  ...
  async unlinkAccount({ provider, providerAccountId }) {
    return (
      (await callMutation(api.authAdapter.unlinkAccount as any, {
        provider,
        providerAccountId,
      })) ?? undefined
    );
